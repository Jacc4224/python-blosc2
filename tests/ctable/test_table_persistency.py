...
def test_open_wrong_kind_raises(tmp_path):
    """A path whose _meta has kind != 'ctable' raises ValueError."""
    # Build a real table structure then overwrite the kind marker.
    path = str(tmp_path / "wrong_kind")
    t = CTable(Row, urlpath=path, mode="w", expected_size=4)
    t.append((1, 1.0, True))
    del t

    from blosc2.ctable_storage import FileTableStorage

    storage = FileTableStorage(path, "a")
    meta = storage._open_meta()
    meta.vlmeta["kind"] = "something_else"
    del storage
